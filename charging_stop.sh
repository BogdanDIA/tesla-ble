...
charging_stop()
{
  app() {
    ...
    # run command                                                                   
    OUT=$(./tesla-control $CMD_TMO -vin "$VIN" -key-file "$PRIVATE_KEY" -ble charging-stop)
    STATUS=$?
...
timeout --preserve-status -k 1 -s SIGKILL "$COMMAND_TIMEOUT" bash -c ". ${SCRIPTS_PATH}/tesla-ble.conf; charging_stop"
STATUS=$?
wait

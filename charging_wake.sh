...
charging_wake()
{
  app() {
    ...
    # run command
    OUT=$(./tesla-control $CMD_TMO -vin "$VIN" -key-file "$PRIVATE_KEY" -ble wake)
    STATUS=$?
...
timeout --preserve-status -k 1 -s SIGKILL "$COMMAND_TIMEOUT" bash -c ". ${SCRIPTS_PATH}/tesla-ble.conf; charging_wake"
STATUS=$?
wait
...
if [[ $STATUS -eq 0 ]]; then
  log "Command wake success"
elif [[ $STATUS -eq 137 ]]; then
  log "Fail - Command Timeout"
else
  log "Command wake fail"
fi
exit "$STATUS"

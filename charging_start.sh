...
{
  app() {
    ./tesla-control -vin "$VIN" -key-file "$PRIVATE_KEY" -ble charging-start
    STATUS=$?
    return $STATUS
  }
...
timeout --preserve-status -k 1 -s SIGKILL "$COMMAND_TIMEOUT" bash -c ". ${SCRIPTS_PATH}/tesla-ble.conf; charging_start"
STATUS=$?
wait
...
if [[ $STATUS -eq 0 ]]; then
  log "Command charging-start success"
elif [[ $STATUS -eq 137 ]]; then
  log "Fail - Command Timeout"
else
  log "Command charging-start fail"
fi
exit "$STATUS"
